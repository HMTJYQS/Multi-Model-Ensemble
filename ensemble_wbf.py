...
def get_weighted_box(boxes,conf_type='avg'):
    ...
    box[0] = boxes[0][0]
    if conf_type == 'avg':
        box[1] = conf / len(boxes)#box[1]是confidence
    elif conf_type == 'max':
...
def find_matching_box(boxes_list, new_box, match_iou):
    ...
    for i in range(len(boxes_list)):
        box = boxes_list[i]
        if box[0] != new_box[0]:#判断weight_boxes里的框是否和未匹配的框label相同
            continue
...
def weighted_boxes_fusion(boxes_list, scores_list, labels_list, weights=None, iou_thr=0.55, skip_box_thr=0.0, conf_type='avg', allows_overflow=False):
    ...
    if weights is None:
        weights = np.ones(len(boxes_list))#boxes_list是那几个待融合的模型
    ...
    overall_boxes = []
    #filtered_boxes是个dict,里面包含的是每一个label这几个模型一共有几个框，还没找到相互匹配的框呢
    for label in filtered_boxes:# [int(label), float(score), weights[t], t, x1, y1, x2, y2]
        boxes = filtered_boxes[label]#拿出来这一类的所有的框
        new_boxes = []
        weighted_boxes = []
        # Clusterize boxes 把框一个一个拿出来做匹配，聚类出同一个目标的框
        ...
        for i in range(len(new_boxes)):
            ...
            if conf_type == 'box_and_model_avg':
                ...
                #weights.sum()就是论文里的N，len(clustered_boxes)是T
